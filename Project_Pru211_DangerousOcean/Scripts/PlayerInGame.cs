...
public class PlayerInGame : MonoBehaviour
{
    ...
    void Update()
    {
		...
		*/
		...
		//Debug.Log(dir);
		//float test3 = this.transform.position.y;
		float[] t = getAngle(dir);
		Vector3 test = new Vector3(90,0,t[0]);
		this.transform.eulerAngles = test;
		//this.transform.position = new Vector3(this.transform.position.x,test3,this.transform.position.z);
	...
	public void toGame(int die)
	{
		GameObject tmp = GameObject.Find("Balade");
		player.gold += money;
		player.bestScore = (player.bestScore > money)?player.bestScore:money;
		player.numberOfDivingDone += 1;
		player.bestDepth = (player.bestDepth > -this.gameObject.transform.position.z)?player.bestDepth:-this.gameObject.transform.position.z;
		player.numberOfMeterDive += this.gameObject.transform.position.z;
		player.numberOfPredatorHunt += nbrSharkKill;
		switch (die)
		{
			case 0:
				player.numberOfDeathByPredator += 1;
				break;
			case 1:
				player.numberOfDeathByAsphixy += 1;
				break;
			default:
				player.numberOfDeathByObstacle += 1;
				break;
		}
		player.saveData();
		Debug.Log(tmp);
